...
>
> 配套执行文件：[《实施切片与验收手册 v0.2》](./海事离线AI中枢_实施切片与验收手册_v0.2.md)

**v0.2 变更摘要**（对照赛事评审标准的增量补丁）：

1. 新增 §2.4 评分对齐矩阵：六项官方评审标准逐项映射到项目载体。
2. 模型选型全面锁定 **NVIDIA + Stepfun（阶跃星辰）双官方开源生态**，新增 §4.1 模型选型与内存预算表。
3. 架构表述升级为**多智能体（Agent）协同**：新增引用审计 Agent、身份富化 Agent、语音播报 Agent。
4. LLM 解释与审计升格为 **MVP 必做项**（评分关键路径），模板回退降级为运行时保险。
5. 新增微调切片 SF1（检测器域适应，前后对比评测），满足"模型调优深度"评分项。
6. 确认 **FVessel 公开数据集**为同步 AIS–视频的保险数据源与定量评估集。
7. 新增 72 小时后的 D4–D10 排期（微调、能力演示、Demo 视频制作、十日谈征文）。
...
- 系统以多智能体（Agent）架构表述与实现：各 Agent 有明确职责契约、独立激活模式与可审计的输入输出。
- FVessel 公开基准数据集（同步 AIS + 视频 + 相机标定 + 融合真值）作为 G0 保险数据源与定量评估集；自有同步样本作为 demo 叙事首选。
- LLM 解释与引用审计为 MVP 必做项（评分关键路径）；十万级船舶库通过身份富化 Agent 进入融合证据链。
...
|---|---:|---|---|---|
| 项目实用性、行业落地价值与技术创新性 | 25% | 外海弱网 SA 真实痛点；异构证据互验 + 诚实降级的创新架构；商业落地路径（§21 五级） | §3、§4、§21 | 主干不可裁剪 |
| 智能体融合与模型优化技术深度 | 25% | 七 Agent 协同（§4.1）+ 引用审计 Agent 的双 LLM 协作；SF1 检测器微调前后对比（调优深度） | §4.1、手册 SF1/S6 | **S6 双 Agent 与 SF1 不可裁剪** |
| 项目完整性 | 20% | MapLibre 前端 + WebSocket 后端；切片验收证据体系；一键启动/重置演示 | 手册 §3、S7 | 不可裁剪 |
| 平台适配性 | 15% | 全栈 NVIDIA（TAO 微调、TensorRT/vLLM 推理、Nemotron 系列）+ Stepfun（Step-Audio 2 mini、Step-Audio-TTS-3B）；Spark 单机训练+推理 | §4.1、手册 SP0/SF1 | **双生态出场不可裁剪** |
| 演示效果（Demo 视频） | 10% | 90 秒主闭环 + iptables 断网高光 + 微调前后对比镜头 + 能力演示段 | 手册 V1 | 不可裁剪 |
| 赛事征文（十日谈） | 5% | 每日开发日志（验收证据目录即素材），D9 汇编 | 手册 E1 | 不可裁剪 |

> 解读：第 2、4 两项合计 40% 的权重完全依赖 v0.2 新增的 SF1（微调）、S6 双 Agent（解释+审计）与双生态模型出场——这三者是"不做就直接丢分"的硬载体，优先级等同主干。
...
```mermaid
flowchart LR
    AIS["本船 AIS 只读输出<br/>周边船舶广播"] --> TRACK["AIS 瞭望 Agent<br/>解码、轨迹、新鲜度、CPA/TCPA<br/>（确定性）"]
    PNT["本船 GPS / 航向 / 航速"] --> TRACK
    CAM["船外视频"] --> VISION["视觉瞭望 Agent<br/>检测、跟踪、方位<br/>（NVIDIA TAO 检测器）"]
    MIC["船外声响"] --> AUDIO["听觉瞭望 Agent<br/>声号分类<br/>（Stepfun Step-Audio 2 mini）"]
    ENC["本地海图 / 缓存天气 / SOP"] --> CONTEXT["航行上下文与约束"]
    FLEET["十万级船舶库"] --> ENRICH["身份富化 Agent<br/>MMSI→注册船型交叉验证<br/>（确定性查询）"]

    TRACK --> FUSION["融合仲裁 Agent<br/>时空对齐、门控、打分<br/>（确定性）"]
    VISION --> FUSION
    AUDIO --> FUSION
    ENRICH --> FUSION
    VISION -. 按 encounter 触发 .-> VLM["场景理解 Agent<br/>粗船型/场景描述<br/>（Nemotron Nano 12B v2 VL）"]
    VLM -. 证据分量 .-> FUSION
    CONTEXT --> FUSION

    FUSION --> RISK["风险哨兵 Agent<br/>规则优先、版本化阈值<br/>（确定性）"]
    RISK --> EXPLAIN["解释 Agent<br/>依据与核查清单<br/>（Nemotron，本地 RAG）"]
    EXPLAIN --> AUDITOR["引用审计 Agent<br/>数字/引用溯源校验<br/>（Nemotron Nano 9B v2）"]
    AUDITOR --> UI["船长界面<br/>提示、证据、人工确认"]
    RISK -. 告警不等待 LLM .-> UI
    UI -. 可选语音播报 .-> TTS["语音播报 Agent<br/>（Stepfun Step-Audio-TTS-3B）"]
    UI --> AUDIT["本地事件记录与审计"]
```
...
- **多智能体协同**：每个 Agent 有明确的职责契约（输入 schema、输出 schema、激活条件、失败行为），Agent 间通过共享结构化状态通信而非自由文本传递；解释 Agent 与引用审计 Agent 构成生成—校验的双 LLM 协作，任何未通过审计的解释不得上屏。
...
### 4.1 模型选型与内存预算（v0.2 新增，全部锁定 NVIDIA + Stepfun 官方开源生态）

| Agent | 职责 | 模型 / 实现 | 生态 | 量化与内存估算 | 激活模式 |
|---|---|---|---|---|---|
| AIS 瞭望 | 解码、轨迹、CPA/TCPA | 确定性逻辑 + pyais 库 | — | 可忽略 | 常驻，事件驱动 |
| 视觉瞭望 | 船舶检测与跟踪 | NVIDIA TAO 预训练检测器（DINO 系）+ ByteTrack，导出 TensorRT 引擎；SF1 微调对象 | NVIDIA | 引擎约 1–3 GB | 常驻，逐帧 |
| 场景理解 | 粗船型 / 场景描述证据 | Nemotron Nano 12B v2 VL（NVFP4-QAD，社区已有 Spark/vLLM 运行手册） | NVIDIA | 约 7–9 GB | 按 encounter 触发，不逐帧 |
| 身份富化 | MMSI→注册船型/尺寸交叉验证 | 确定性查询（十万级船舶库） | — | 可忽略 | 按目标触发 |
| 融合仲裁 | 门控、打分、状态 | 确定性逻辑 | — | 可忽略 | 事件驱动 |
| 风险哨兵 | 版本化规则、事件状态机 | 确定性逻辑 | — | 可忽略 | 事件驱动 |
| 解释 Agent | 告警解释、核查清单 | 默认 Nemotron-3-Nano-30B-A3B（MoE 稀疏激活，NVFP4，解码快、契合带宽约束）；备选 Nemotron Nano 9B v2（官方 DGX Spark NIM 容器） | NVIDIA | NVFP4 约 16–18 GB / 约 6–8 GB | 按告警触发，≤150 token 流式 |
| 引用审计 | 解释中数字/引用溯源校验 | Nemotron Nano 9B v2（独立小模型；内存吃紧时可与解释模型共权重、异系统提示，须如实标注） | NVIDIA | FP8/NVFP4 约 6–9 GB | 解释生成后触发 |
| 听觉瞭望 | 雾号等规则化声号分类 | Step-Audio 2 mini（端到端音频理解，可处理非语音信号，ModelScope 开源） | Stepfun | 约 8–16 GB | A1 能力演示时段加载 |
| 语音播报 | 告警语音（可选加分项） | Step-Audio-TTS-3B（ModelScope 开源） | Stepfun | 约 4–6 GB | 按告警触发，失败静默降级 |

内存纪律：常驻核心（检测引擎 + 解释 + 审计）约 25–30 GB；全量峰值（含 VLM、音频、TTS 同时加载）约 55–65 GB，远低于 128 GB 统一内存的 80% 上限。**错峰激活**是硬约束的应对：视觉逐帧持续、不能让路；LLM 全部按事件触发、短输出、流式；同一时刻至多一个大 LLM 处于解码状态。所有估算以 SP0 实测为准并写入 `configs/models.yaml`（含生态归属字段）。

> 型号说明：以上型号均已核实为官方开源且有 Spark/aarch64 可行性证据（Nemotron Nano 9B v2 有 DGX Spark 专用 NIM 容器；Nemotron 系列有官方 Spark 部署指南；Nano 12B v2 VL 的 NVFP4 量化版有社区 Spark 运行手册；Step-Audio 2 mini 与 TTS-3B 在 ModelScope 开源）。SP0 若实测某型号不可用，按同生态内就近降级并记录，不引入第三方生态模型。
...
- 有限集合的规则化号笛模式识别（听觉瞭望 Agent，Step-Audio 2 mini）；
- 左/右、前/后或方位扇区级声源方向（**仅在取得真实麦克风阵列/多通道数据时**；公开单通道录音只做类型识别，不做方向）；
- 与同时间 AIS 方位、视频目标的交叉验证（仅在取得同步音频时）；
- 证据不一致时的人工瞭望请求。

v0.2 明确音频的两条路径：**同步路径**（取得与主场景时间对齐的船外音频→进入融合主张）与**能力演示路径 A1**（使用公开真实船舶声号录音，明确标注"与主场景未同步"，仅演示 Step-Audio 2 mini 的声号分类能力，不进入融合主张）。无论哪条路径，Stepfun 音频模型都必须出场（评分对齐，§2.4）。
...
- 至少一个 AIS–视频目标关联场景（融合证据含身份富化 Agent 的船库交叉验证）；
- 证据、置信度、数据年龄和降级状态的可视化（MapLibre 态势界面，本地底图断网可渲染）；
- 风险提示、人工确认与本地事件审计；
- **解释 Agent + 引用审计 Agent 双 LLM 闭环**（评分关键路径，不可裁剪；模板回退仅为运行时保险）；
- **SF1 检测器微调**：在冻结验证集上给出微调前后对比（评分关键路径，不可裁剪）；
- **双生态模型出场**：NVIDIA（检测/VLM/解释/审计）与 Stepfun（音频/TTS）各至少一个模型在最终演示中真实运行；
...
- 语音播报 Agent（Step-Audio-TTS-3B）作为主线可选加分项，失败静默降级。
...
| 模型调优（SF1） | 同一冻结验证集上微调前后 mAP / 召回 / 误报对比（绝对值 + 相对值并报），训练在 Spark 完成的资源记录 |
| 解释与审计 | 解释首个有用内容 P95 延迟、输出 token 预算符合率、审计引用溯源通过率、注入对抗样本拦截率 |
...
1. **没有可合法使用的同步 AIS–视频样本**：无法证明核心融合能力。→ v0.2 缓解：FVessel 公开数据集为保险数据源（岸基固定相机场景，含标定与融合真值）；触发保险时"本船"退化为位置已知的固定相机，属算法更简单的特例，对外叙事须如实说明。
...
7. **评分关键载体缺席**（v0.2 新增）：SF1 微调、S6 双 Agent、双生态模型出场三者中任一缺席，将直接损失"智能体融合与模型优化 25%"与"平台适配 15%"的对应权重——它们与主干同级，不得因进度压力裁剪。
...
- 解释 Agent 在带宽受限下可能超出延迟预算：应对为 ≤150 token 输出上限 + 流式渲染 + MoE 稀疏激活模型优先；指标以"首个有用内容时间"为准。
...
### 72 小时之后：D4–D10 排期（v0.2 新增，覆盖全部十天赛程）

| 日程 | 主要工作 | 对应评分项 |
|---|---|---|
| D4 | SF1 微调训练启动（长任务尽早发射）；S6 双 Agent 打磨与对抗注入测试 | 智能体融合与调优 25% |
| D5 | SF1 验收（前后对比、TensorRT 引擎回填 S3）；A1 音频能力演示（Step-Audio 2 mini） | 调优 25% + 平台 15% |
| D6 | 定量评估补样本（FVessel 10+ encounter）；性能压测与内存基线复测 | 实用性 25% + 完整性 20% |
| D7 | 全链路彩排（一键启动/重置 ×3）；缓冲与缺陷修复 | 完整性 20% |
| D8 | V1 Demo 视频制作（脚本、录制、剪辑、配音） | 演示效果 10% |
| D9 | E1 十日谈汇编成文；提交包按 §2.4 矩阵逐项自检 | 征文 5% + 全项 |
| D10 | 缓冲、最终提交 | — |

> 十日谈日志从今天（D3）起每日记录并回补 D1–D2；验收证据目录（`results/acceptance/`）即日志素材，不额外增负。
...
> 全部模型来自 NVIDIA 与阶跃星辰官方开源生态：视觉检测用 NVIDIA TAO 微调并以 TensorRT 部署，场景理解与解释/审计用 Nemotron 系列，听觉理解与语音播报用 Step-Audio 系列；检测器微调与七个 Agent 的全部推理在同一台 DGX Spark 上完成——128 GB 统一内存让"训练 + 多模型常驻推理"在单机闭环。
