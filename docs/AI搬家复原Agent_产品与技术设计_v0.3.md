...
> **2026-07-16 初赛范围覆盖：**Sean 核对参赛指导后确认，初赛提交物为成果、
> 演示视频与十日谈。项目收敛为一个受控英雄任务；正式任务 B 盲测、多房间泛化、
> 5000 张网络/生成图片、R1 主动学习与 Grounding DINO 全参数微调停止。
> 本文相冲突处以 [`初赛范围冻结_2026-07-16.md`](./初赛范围冻结_2026-07-16.md) 为准。
...
初赛只建立一个授权、整洁、可复跑的英雄搬家任务。该任务允许用于词表、阈值、
轻量投影头、规则和 UI 调试；不再建立正式任务 B，也不据此宣称跨房间泛化。
报告模型指标时仍按完整视频划分训练视角与验证视角，避免相邻帧泄漏。
...
| 重复运行确定性 | 冻结输入至少完成一次稳定复跑，实体与布局结果一致 | 生成式文案可不同，事实不得漂移 |
...
1. 冻结视觉骨干建立基线；
2. 用受控英雄任务的多视角正样本和相似物品负样本训练轻量投影头；
3. 优先使用 supervised contrastive loss 或 triplet loss；
4. 困难负样本必须包含同色、同类别、不同实例的物品；
5. 只使用受控英雄任务按完整视频划分的验证结果选择最终模型和阈值；
6. 报告同一受控任务内的基线与调优结果，并明确它不是跨房间泛化结论。
...
| 能力 | 首选路径 | 七日训练/优化 |
|---|---|---|
| 开放词汇检测 | Grounding DINO（社区 checkpoint）；SF1-L2 走 ONNX→TensorRT **部署**（口径不称 TAO） | 初赛冻结 checkpoint、词表和阈值，不训练 GDINO 权重 |
...
这些链接只证明相邻能力和工具链存在，不证明本项目的跨视频实体闭环或生活复原效果已经完成；初赛结论只以受控英雄任务的真实本地产物、复跑记录和演示证据为准，不外推为跨房间泛化。
