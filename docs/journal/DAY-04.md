...
- 启动 S3 跨视频匹配关键路径，用 `dev-a-vocab5` 四视频现有产物形成可复跑基线，并逐组检查 A–D 四组同类不同实例硬负样本。
- 落地 S2.5 推理侧核心修复，冻结参数后只重跑一次 `dev-a-vocab6`，与 v5 对照。
- 将 `vlm_attributes` 主路切换为 Nemotron VL NVFP4-QAD/vLLM，保留 BF16 fallback，并裁撤独立 9B `task_copy` 槽位。
- 将 A1 评价目标从额度消耗改为覆盖完整性与统计稳定性，建立无需人工逐条标注的
...
- 未验证或降级边界：
  ...
  - S2.5-6 云视觉教师/增强数据工厂在未得到本批影像出境明确同意前，不会上传家庭 crop；不阻塞 v6 主链。
  ...
  - 完整 ReID 用冻结四点网格 `match=0.80/0.82/0.84/0.86, margin=0` 校准，选择 0.80；
    ...
    `156→257`、澄清 `525→505`。但总门仍仅 `5/17 < 15/17`，因此仅记为
...
- 按冻结的 `configs/sf1_hero_s1.yaml` 用正式 anchor 标注重训 SF1-L1，不沿用 dev_a 权重；
...
- **两份数据所有者真值均已落地**：17 锚点确认(161 轨,零重复归属、零硬负冲突,
...
- **GT 授权的阈值扫描**(12 组合 match×margin):全网格误合并=0、危险澄清恒=4;
  ...
  **v3 冻结待人工目检 71 对新增链接**(`new-links-m084g000.jpg`;锚点 GT 只
...
- **71 对新增链接人工目检：71/71 对照正确** → v3 最后一道门解除。
...
- **范围前提**：我认为初赛应聚焦单一受控英雄卧室任务，并据此形成
  《初赛范围冻结 2026-07-16》；dev_a 降级为诊断证据，属性接线属"继续完成"
...
- **建议**：v3 仍为 dev_a 冻结口径；attr 0.15 作为英雄场景候选工作点
...
- **GROUP 取舍**：不依赖“画面空间邻近聚类”——移动镜头二维远近≠真实
...
- **A1 本地化方案收官**：spark 建 stepaudio 独立 env,官方报文构造内联
  ...
  (0.2/贪心 × 零-shot/few-shot)结论一致——**本地协议收敛为零-shot 贪心**
  (判卷 1/3,label 3/3 全对,失分全在槽位保真);few-shot 两温度均 0/3
  且稳定幻化打包搭子“冬天的衣服”成独立物品，因此不采用。定位结论:mini 边界=
  "听得准认得出",槽位结构化不达云参考 → 主链走"本地誊写+确定性解析器",
  mini 结构化只作对照。判断全文写入 `results/stepfun/a1_warmup/PROTOCOL.md`。
  并行建设 TTS 语音鲁棒性测试农场（云 TTS 造噪声/口音变体）补判据。
