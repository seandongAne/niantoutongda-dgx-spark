# 海事离线 AI 中枢：实施切片与验收手册 v0.2

> 配套方案：[《海事离线 AI 中枢：项目定义与验证计划 v0.2》](./海事离线AI中枢_项目定义与验证计划_v0.2.md)
>
> 队伍：念头通达
>
> 文档日期：2026-07-14（v0.2 增量修订）
>
> 用途：把项目愿景转成十天赛程内可逐段交付、现场共同验收、失败时可降级的工程计划

**v0.2 变更摘要**：新增 1.5 评分关键项不可裁剪原则；S6 升格为解释+审计双 Agent 主线切片并锁定 Nemotron 型号；新增 SF1 微调切片、A1 音频能力演示、V1 视频制作、E1 十日谈四个 D4–D10 切片；SP0 增加双生态（NVIDIA + Stepfun）模型启动验证与 NGC 容器纪律；S3 后新增 S3-SP Spark 检查点，废除"最后 6 小时大爆炸迁移"；S2 明确 pyais + MapLibre + 本地底图与身份富化 Agent；S7 增加 iptables 断网证明；验收门槛表相应更新。
...
4. AIS–视频关联；
5. 风险事件、人工确认和审计；
6. 解释 + 引用审计双 Agent（主线，见 1.5）；
7. 检测器微调（SF1）与音频、天气等能力（D4 起）。

LLM 不参与 AIS 解码、CPA/TCPA 数值计算、目标身份绑定或告警阈值判断。它只读取已经形成的结构化事件，负责解释依据、检索规则与生成核查清单；引用审计 Agent 再对解释做溯源校验。
...
**主干必须完成：** AIS 回放→轨迹→视频检测→目标关联→风险提示→人工确认→审计→**解释 + 引用审计双 Agent**（v0.2：双 Agent 属主干，见 1.5）。

**评分切片必须完成（D4 起）：** SF1 检测器微调、A1 音频能力演示、V1 视频、E1 十日谈。

**条件满足后再加入（融合主张）：** 同步船外音频（O1）、天气危险窗口（O2）。
...
### 1.5 评分关键项不可裁剪（v0.2 新增）

赛事评审中"智能体融合与模型优化技术深度（25%）"与"平台适配性（15%）"合计 40% 的权重，完全由以下三个载体承接，**任何进度压力下不得裁剪，优先级等同主干**：

1. **S6 解释 + 引用审计双 Agent**：系统中 LLM 协作的唯一出场点；模板回退仅为运行时故障保险，不是计划选项。
2. **SF1 检测器微调**：唯一的"模型调优深度"证据；无提升也须如实报告负结果，负结果本身即调优深度证据。
3. **双生态模型出场**：NVIDIA（TAO 检测器、Nemotron 系列）与 Stepfun（Step-Audio 2 mini、Step-Audio-TTS-3B）各至少一个模型在最终演示中真实运行，型号与版本写入 `configs/models.yaml`。

主干（S1–S5、S7）保障"项目完整性 20%"与"实用性 25%"；若主干与评分关键项发生资源冲突，按 §2.4 评分矩阵的权重损失量化后再裁决，不凭直觉砍 LLM。
...
flowchart LR
    ...
    S5 --> S6["S6 解释+审计双 Agent（主线）<br/>58–66h"]
    S6 --> S7["S7 Spark 离线验收与演示固化<br/>66–72h"]
    S3SP --> S7
    SP0 --> S7
    S7 --> SF1["SF1 检测器微调<br/>D4–D5"]
...
| S3-SP | Spark 检查点（v0.2 新增） | S3 冻结样本在 Spark 上复跑验收达标 | S3 | 立即触发 S3 降级序列，不得推迟到 S7 |
| S4 | AIS–视频融合 | 显示匹配、未匹配、冲突及证据贡献（含船库/VLM 分量） | S2、S3 | 不得称为多模态融合 |
| S5 | 人工在环闭环 | 告警→人工确认→态势更新→留痕 | S4 | G4 失败，不能形成 90 秒闭环 |
| S6 | 解释+审计双 Agent（主线，不可裁剪） | 解释"为什么提醒"+ 审计徽章，未过审计不上屏 | S5 | 运行时故障才回退模板；计划层面不可移除 |
| S7 | 离线与演示固化 | iptables 断网证明下连续、可复现地跑完整闭环 | S5、S6、S3-SP、SP0 | 降并发模型并保留主干，不能伪装离线 |
| SF1 | 检测器微调（v0.2 新增，不可裁剪） | 同一冻结验证集上的微调前后对比 | S3-SP、检测训练集就绪 | 减 epoch→只调检测头→如实报告负结果 |
| A1 | 音频能力演示（v0.2 新增） | Step-Audio 2 mini 对公开真实声号的分类演示 | 公开真实录音就绪 | 缩小类别集合；Stepfun 出场由 TTS 兜底 |
| V1 | Demo 视频制作（v0.2 新增） | 3–4 分钟成品视频 | S7、SF1 | 用 S7 录屏 + 旁白的简版兜底 |
| E1 | 十日谈征文（v0.2 新增） | 每日日志 + D9 汇编成文 | 即日启动 | 无（成本极低，无裁剪理由） |
| O1 | 音频增强（融合主张） | 号笛类型/方向成为独立证据 | 同步真实音频准入 | 降级为 A1 能力演示 |
...
| 解释首个有用内容 | P95 ≤2 s（流式渲染） | 替代原"解释 P95 ≤5 秒"单一指标；完整解释 ≤8 s |
| 解释输出预算 | ≤150 token/条 | 超限视为违规；追问再展开，不一次吐长文 |
| 引用审计溯源 | 解释中数字/规则引用 100% 可溯源到结构化事件或知识库 | 未过审计的解释不上屏 |
| 对抗注入拦截 | 20 例含伪造数字/无据引用的注入解释，拦截 20/20 | 审计 Agent 的独立验收 |
| 微调对比（SF1） | 同一冻结验证集，绝对值 + 相对值并报 | 不得换验证集；负结果如实报告 |
...
出现硬停止条件时，不允许用手工移动 AIS 点位伪造融合。**v0.2 保险路径**：切换到 FVessel 公开基准数据集（26+ 段视频 + 同步 AIS CSV + 完整相机参数文件 + 逐秒检测/跟踪/融合真值，长江武汉段岸基相机采集，IEEE TITS 2023 论文配套公开）。切换的含义与边界：
...
6. **Stepfun 出场验证**：从 ModelScope 拉取 Step-Audio 2 mini 与 Step-Audio-TTS-3B，各完成一次可复现推理（一段音频分类 / 一句告警播报）。
...
- **NVIDIA 与 Stepfun 双生态各至少一个模型在 Spark 上完成一次可复现推理**；
...
8. 实现身份富化 Agent：MMSI → 十万级船舶库查询 → 注册船型/尺寸 → 输出 `fleet_class_prior` 字段供 S4 打分（查询失败输出 `ENRICH_UNAVAILABLE`，不阻塞轨迹）。
...
### S3-SP：Spark 检查点（40–44 小时，v0.2 新增）
...
`coarse_class_consistency` 由两路独立证据构成（v0.2）：

- **船库先验**：身份富化 Agent 输出的注册船型/尺寸（确定性，来自十万级船舶库）；
- **视觉粗分类**：场景理解 Agent（Nemotron Nano 12B v2 VL）对该 encounter 目标框的粗船型判断——**按 encounter 触发一次，不逐帧调用**；VLM 结论只作为证据分量参与打分，不得单独绑定身份，VLM 不可用时该分量记为缺失并重新归一化。
...
8. （可选加分，不阻塞）语音播报 Agent：告警文本经 Step-Audio-TTS-3B 生成语音播报；TTS 失败或超时静默降级为纯视觉告警，不得影响告警时延。
...
## 11. S6：解释 + 引用审计双 Agent（58–66 小时，主线，不可裁剪）

### 用户可见结果

船长可询问“为什么提示我”“哪些数据不确定”“应检查什么”，系统依据结构化事件、COLREG 摘要和船方 SOP 给出可追溯解释；每条解释附带**审计徽章**——引用审计 Agent 校验通过才上屏。

### 双 Agent 架构与模型（v0.2）

- **解释 Agent**：默认 Nemotron-3-Nano-30B-A3B（NVFP4，MoE 稀疏激活、解码快，契合带宽约束）；备选 Nemotron Nano 9B v2（官方 DGX Spark NIM 容器）。按告警触发，输出 ≤150 token、流式渲染；追问再展开。
- **引用审计 Agent**：Nemotron Nano 9B v2（独立小模型；内存吃紧时可与解释模型共权重、异系统提示，须在文档与演示中如实标注）。职责：逐项校验解释中的每个数字能在告警事件 JSON 中精确匹配、每条规则引用能命中本地知识库条目；任一违规→打回重生成一次→仍违规→输出 `INSUFFICIENT_EVIDENCE` 并回退确定性模板。
- 审计**不阻塞告警展示**：告警的确定性字段（目标、CPA/TCPA、数据年龄）即时上屏；解释流式补充；审计徽章异步点亮。
...
source_refs[]
prohibited_actions[]
audit_status          # v0.2：PASSED / REGENERATED / FALLBACK_TEMPLATE
```
...
- 每项规则性解释带本地来源引用；
- 引用审计溯源通过率 100%：未过审计的解释 0 条上屏；
- **对抗注入测试**：预设 20 例含伪造数字或无据引用的注入解释，审计 Agent 拦截 20/20；
- 缺少依据时输出 `INSUFFICIENT_EVIDENCE`；
- 20 个安全测试中 0 次输出直接操船指令；
- 模型超时、崩溃或未加载时，核心告警不受影响，并回退到确定性模板（`audit_status=FALLBACK_TEMPLATE` 如实显示）；
- 解释首个有用内容 P95 ≤2 秒（流式），完整解释 ≤8 秒，输出 ≤150 token；
- 用户界面明确区分“传感器事实”“系统推断”“规则说明”。

### 优先级裁决（v0.2 修订）

S6 是评分关键路径（§1.5），**计划层面不可裁剪**。若 S1–S5 未稳定挤占了 S6 时间，正确动作是：先保证双 Agent 在**单一冻结告警**上端到端可演示（哪怕只有一个场景），再回头加固主干——而不是移除 S6。确定性模板仅作为运行时故障保险存在。
...
2. 在模型加载前执行 `free -h` 并记录基线；随后逐个加载常驻模型（检测引擎→解释→审计），形成**全模型内存基线表**（每步增量 + 峰值）写入 `manifest.json`。
...
## 12A. SF1：检测器微调切片（D4–D5，v0.2 新增，不可裁剪）

### 目标

以可复核的前后对比满足"模型调优深度"评分项，同时把检测器域适应到海事场景（雾天、低光、小目标、水面反光）。训练在 Spark 上完成——"同一台机器训练 + 推理"本身即平台全栈证据。

### 准入条件

- S3-SP 检查点通过（基线模型与验证集已冻结）；
- 检测训练集就绪：FVessel 检测图像集（7625 + 3728 张带标注图像）与自有标注数据；
- 冻结验证集划分完成并登记校验值（训练开始后不得变更）。

### 实施任务

1. 以 S3 冻结的 TAO 预训练检测器为基座，在 Spark 上启动微调（TAO Toolkit 或 NeMo/PyTorch 路线，D4 上午即发射——这是全程最长的单一任务，`nohup` 后台 + 日志轮询）。
2. 训练资源留证：`nvidia-smi` 采样、峰值内存、训练时长写入证据目录。
3. 微调完成后在**同一冻结验证集**上评测：mAP、召回、误报/分钟，整体 + 雾天/低光子集，绝对值与相对值并报。
4. 选取 3–5 个错误案例做前后对比图（可直接进 V1 视频素材）。
5. 达标则导出 TensorRT 引擎回填 S3 检测链路，并复跑一次 S3-SP 验收确认无回归。
6. （可选加深，时间富余才做）解释 Agent LoRA：用 NeMo 框架对结构化输出格式做小规模 SFT，评测 JSON 合规率前后对比。

### 验收规则

- 前后对比在同一冻结验证集上完成，验证集校验值与训练前登记一致；
- 报告绝对数量与相对提升，样本量不足时如实披露；
- **允许负结果**：若微调无提升或劣化，如实报告并附原因分析——负结果本身即调优深度证据，禁止换验证集或挑样本制造提升；
...
3. 时间耗尽仍未收敛：以已完成的训练记录 + 中间评测如实报告，不虚构最终结果。

---

## 12B. A1：音频能力演示（D5，v0.2 新增）
...
- 若后续取得同步音频（O1 准入达成），A1 结论可作为模型选型依据升级进 O1，但融合主张仍须按 O1 验收重新落证。
...
| 架构与双生态 | ~30 s | 七 Agent 架构图；模型清单动画（NVIDIA：TAO/Nemotron 系列；Stepfun：Step-Audio 系列）；"单机训练 + 推理" |
| 90 秒主闭环 | ~90 s | S7 固化的完整回放：离线状态→AIS 态势→视频检测→融合→告警→解释流式 + 审计徽章→人工确认→审计留痕 |
| 断网高光 | ~15 s | iptables 出站封锁规则特写 + 系统持续运行 |
| 微调对比 | ~20 s | SF1 前后错误案例对比图 + 指标条 |
| 能力演示 | ~15 s | A1 声号分类（未同步标注在画面上）+ TTS 播报一条告警 |
...
- 旁白可用 Step-Audio-TTS-3B 生成部分段落（自然呼应平台叙事）；
- D7 彩排时即按分镜预演一遍，D8 只做录制与剪辑；
- 简版兜底：若 D8 时间告急，S7 三次成功回放录屏 + 字幕旁白即为最低可交付版本。

---

## 14B. E1：十日谈征文（即日启动，D9 汇编，v0.2 新增）

- 每日一条日志：`docs/journal/DAY-XX.md`，固定模板（今日目标 / 关键证据或截图 / 失败与教训 / 明日计划），5–10 分钟完成；
- 素材零增负：验收证据目录（`results/acceptance/`）、失败案例文件、metrics 即日志素材，直接引用；
- 今天是 D3：今日补齐 D1（环境与骨架）、D2（选题收敛）后开始正常记录；
- D9 汇编成文：以"生死门→切片推进→微调→固化"为叙事线，如实呈现失败与降级——诚实的失败记录比顺风叙事更有征文价值；
- 责任：每个切片主责人写当日相关段落，UI/助手与演示负责人汇编。

---

## 14C. D4–D10 总排期（v0.2 新增）

| 日程 | 上午 | 下午/晚间 | 里程碑 |
|---|---|---|---|
| D4 | SF1 训练发射（最长任务最先跑） | S6 双 Agent 对抗注入测试打磨 | SF1 训练中；S6 20/20 拦截 |
| D5 | SF1 评测 + 引擎回填 + S3-SP 复验 | A1 音频能力演示 | SF1、A1 验收 |
| D6 | FVessel 评估集扩样（≥10 encounter 定量报告） | 30 分钟稳定性压测 + 内存基线复测 | 定量评估报告 |
...
└── maritime_sa/
    ...
    ├── enrich/             # 身份富化 Agent（船舶库交叉验证）
    ├── fusion/             # 门控、匹配、证据分数（含 VLM/船库分量）、冲突
    ├── risk/               # 版本化规则和事件状态机
    ├── agents/             # 解释 Agent、引用审计 Agent、场景理解 Agent、Agent 契约
    ├── assistant/          # 本地 RAG、模板回退、安全过滤
    ├── audio/              # 听觉瞭望 Agent（Step-Audio 2 mini）、TTS 播报
    ├── storage/            # SQLite 与追加式审计
    ├── api/                # 本地 API / WebSocket
    └── ui/                 # MapLibre 态势、视频、告警与人工确认
docs/
└── journal/                # E1 十日谈每日日志
tests/
├── unit/
...
| AIS/融合负责人 | 事件契约、轨迹、CPA/TCPA、身份富化、关联 | S1、S2、S4 |
| 感知与 Spark 负责人 | 视频/音频、运行时、性能与内存、微调 | SP0、S3、S3-SP、SF1、O1、S7 |
| UI/助手与演示负责人 | 告警流程、双 Agent（解释+审计）、90 秒演示、视频与征文汇编 | S5、S6、S7、V1、E1 |

排期说明（v0.2）：切片小时表按单主力串行编排；若多人并行，S2 与 S3 可在 S1 后分叉同步推进，S4 相应提前——由团队在 G0 验收时按实际人力冻结甘特，不得中途随意改依赖。E1 十日谈为全员每日义务，各切片主责人写当日相关段落。
...
- 十日谈日志每日提交（`docs/journal/DAY-XX.md`），当日事当日记，不留到 D9 追忆。
...
- [ ] 告警、人工确认和审计闭环；
- [ ] 解释 + 引用审计双 Agent 端到端演示（含审计徽章与对抗注入测试记录）；
...
- [ ] SF1 微调前后对比报告（同一冻结验证集，绝对值 + 相对值；负结果如实报告）；
- [ ] 双生态模型清单：`models.yaml`（NVIDIA / Stepfun 型号、量化、版本）+ SP0 双生态启动证据；
- [ ] A1 音频能力演示记录（Step-Audio 2 mini，未同步标注在案）；
- [ ] 3–4 分钟成品 Demo 视频（V1 分镜）；
- [ ] 十日谈全文（D1–D10 日志 + 汇编稿）；
- [ ] 按 §2.4 评分对齐矩阵逐项自检的对照表；
