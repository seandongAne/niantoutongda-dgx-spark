...
>
> 配套执行文件：[《实施切片与验收手册 v0.3》](./海事离线AI中枢_实施切片与验收手册_v0.3.md)

**v0.3 变更摘要**（架构收敛修订；v0.2 摘要见上一版文档）：

1. **架构收敛为 4+1**：十个具名 Agent 收敛为四个异构主链 Agent（AIS 态势、视觉瞭望、融合与风险、船长助手）+ 一个受数据准入控制的声学瞭望 Agent；身份富化、场景理解 VLM、引用校验、TTS 降为 Agent 内部工具。全文废除"七 Agent"表述，统一口径见 §4.0。
2. **引用审计确定性化（转正，非降级）**：引用审计由第二个 LLM 改为**确定性引用校验门**——数字与引用溯源是精确匹配问题，校验器给出构造性 100% 保证，且与"先确定性、后生成式"原则一致。审计徽章、`audit_status`、对抗注入 20/20 测试全部保留；S6 切片不可裁剪地位不变，变的是内部构成。
3. **新增 §4.0 Agent 操作性定义**：五条判据 + "Agent ≠ LLM"，同时回答"为什么只有五个"与"这不就是函数调用吗"。
4. **协同 trace 升格为一等验收产物**：S4 起全部跨 Agent 消息落盘 `agent_trace.jsonl`；融合与风险 Agent 对视觉瞭望 Agent 的复核请求显式化为结构化消息——这是评分 25% 项"多智能体协同"的直接证据。
5. **SF1 扩为三层模型调优**：L1 领域微调（主路径）+ L2 TensorRT 推理优化 + L3 工作点消融；目标三层至少一层正收益；新增**训练/验证按视频段/场景隔离**红线。
6. **Stepfun 主选 Step-Audio 2 mini**；Step-Audio-TTS-3B 降为 SP0 限时保险探针与可选播报适配。
7. 内存预算下调（移除独立审计模型，常驻减 6–9 GB）；修复两文档互链文件名（v0.2 互链指向 `v0.2` 文件名，磁盘实际为 `v0_2`，链接为断链）。
...
| 平台适配性 | 15% | 全栈 NVIDIA（TAO 微调、TensorRT/vLLM 推理、Nemotron 系列）+ Stepfun（**主选 Step-Audio 2 mini**；Step-Audio-TTS-3B 为平台适配保险）；Spark 单机训练+推理 | §4.1、手册 SP0/SF1 | **双生态出场不可裁剪** |
...
**Agent ≠ LLM**：AIS 态势与融合与风险是确定性 Agent，船长助手是生成式 Agent（其审计门仍是确定性工具），声学瞭望是感知模型 Agent——"能力差而非角色差"是本架构的差异化主张。据此，身份富化（按目标查询）、场景理解 VLM（按复核请求推理）、引用校验器（精确匹配）、TTS（输出适配）均为 **Agent 内部工具**：它们无独立状态、不主动发起协作，不冠以 Agent 之名。
...
- **多智能体协同（v0.3 修订）**：4+1 架构，每个 Agent 满足 §4.0 五条判据；Agent 间只通过结构化消息通信，全部跨 Agent 消息落盘 `agent_trace.jsonl`，形成可回放的协同证据链；融合与风险 Agent 在证据模糊或冲突时**主动向视觉瞭望 Agent 发起复核请求**——"一个 Agent 的发现触发另一个 Agent"是本项目多智能体协同的可展示形态；任何未通过确定性引用审计门的解释不得上屏。
...
| **船长助手 Agent** | 告警解释、核查清单、人工确认交互 | 默认 Nemotron-3-Nano-30B-A3B（MoE 稀疏激活，NVFP4，解码快、契合带宽约束）；备选 Nemotron Nano 9B v2（官方 DGX Spark NIM 容器） | NVIDIA | NVFP4 约 16–18 GB / 约 6–8 GB | 按告警触发，≤150 token 流式 |
| └ 工具：引用审计门 | 解释中数字/引用溯源精确校验 | **确定性校验器（零模型）** | — | 可忽略 | 解释生成后同步校验 |
| └ 工具：TTS 播报（可选加分） | 告警语音 | Step-Audio-TTS-3B（ModelScope 开源） | Stepfun | 约 4–6 GB | 按告警触发，失败静默降级 |
| **声学瞭望 Agent（条件准入）** | 雾号等规则化声号分类 | Step-Audio 2 mini（端到端音频理解，可处理非语音信号，ModelScope 开源） | Stepfun | 约 8–16 GB | A1 能力演示时段加载；同步准入通过后进主融合 |

内存纪律（v0.3）：常驻核心（检测引擎 + 解释 LLM）约 **18–21 GB**（v0.2 为 25–30 GB，差额来自移除独立审计模型 6–9 GB）；全量峰值（含 VLM、音频、TTS 同时加载）约 **45–58 GB**，远低于 128 GB 统一内存的 80% 上限。**错峰激活**仍是硬约束的应对：视觉逐帧持续、不能让路；LLM 全部按事件触发、短输出、流式；同一时刻至多一个大 LLM 处于解码状态。所有估算以 SP0 实测为准并写入 `configs/models.yaml`（含生态归属字段）。
...
- 语音播报（船长助手 Agent 的 TTS 适配器，Step-Audio-TTS-3B）作为可选加分项，失败静默降级。
...
| 解释与审计 | 解释首个有用内容 P95 延迟、输出 token 预算符合率、审计门溯源通过率（100%，构造性保证 + 实现验证）、注入对抗样本拦截率（20/20） |
