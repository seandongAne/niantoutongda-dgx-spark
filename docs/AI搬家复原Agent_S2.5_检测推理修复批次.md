...
> **2026-07-16 初赛范围覆盖：**S2.5-6 规模化图片数据工厂、Grounding DINO
> 全参数微调、R1 主动学习与正式任务 B 盲测均已停止。已完成的推理修复和真实判卷
> 继续作为成果证据；未完成扩张项不得再进入初赛关键路径。权威裁决见
> `docs/初赛范围冻结_2026-07-16.md`。
>
...
- 正式任务 B 盲测已取消；若旧素材存在，仍不得反向进入本批次成果或冒充泛化证据。
...
| S2.5-6 | **初赛停止：规模化视觉教师 + 数据工厂** — 不采集 5000 张网络/生成图片，不开 Step Image Edit 训练批次 | 历史设计保留；不再扩展客户端或数据目录 | 不属于初赛验收，不得阻塞主链 |
...
| 项目 | 门条件 | 理由 |
|---|---|---|
| **停止：Grounding DINO 全参数微调** | 初赛不启动，无重新开门条件 | 不搭 TAO/ODVG 训练管道；冻结 checkpoint 与已验证推理配置足以服务受控英雄场景 |
| Mask GDINO / SAM 实例分割 | 同上门;且仅当堆叠问题在瓦片化后仍存在 | codex 自己也排第二阶段 |
| **停止：R1 主动学习** | 初赛不启动，无重新开门条件 | 不建立素材池、challenge、云端增强或循环训练 |
| GDINO TensorRT 化 | **维持 SF1-L2 原排期,不提前**(2026-07-15 拍板) | 帧批处理(S2.5-7)先吃掉最大一口吞吐;TensorRT 引擎构建/校验是独立工程量,提前挤占 Day 4 主链;口径红线同上——没跑就不写 |

### 2.1 R1 主动学习闭环（已停止，历史设计）

以下流程不再执行，仅保留当时的安全设计。除非 Sean 明确重新授权，不得创建任何
R1 素材、challenge、云任务或训练批次。
