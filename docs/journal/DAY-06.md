...
### 增量 D6-12:候选集内多视角 ReID 闭环与诚实关账(深夜)

- 实现提交 `52625dee` 将逐 crop DINOv2 侧车接入 ReID:批量嵌入保持 L2 归一化,
  NPZ 禁用 pickle 并校验格式、维度、连续 view index、有限值、范数和 SHA-256;
  侧车缺轨或 prototype 数量不一致时失败关闭。同视频 stitch 按视角 rank 轮询合并,
  避免长碎轨凭 crop 数量支配代表集。
- baseline 均值向量继续负责双向 Top-5 候选召回,多视角只在该候选并集内重排,不偷偷
  扩大召回。三种集合分数与 raw/projected 两种空间通过独立代理扫描比较;选参脚本
  显式拒绝冻结 anchor GT 路径,并将 tutor 留出桶排除在 winner 选择之外。逐视角产物
  覆盖 4,629 条轨迹、12,232 个视角、缺失 0,SHA-256 为
  `6c48978bf14e5a0a9f63885c4d848f8e56101471f2652efb5665adb7c7a9a00b`。
- 安全优先的 v2 工作点为 `raw/max_pair/blend=0.50`:代理 R@5 从 `0.850267` 增至
  `0.855615`,different-P95 增量 `-0.001573`,但代理 R@1 从 `0.449198` 降至
  `0.443850`;R@1 定向的 v3 提高代理 R@1,但因代理 R@5 退化而单独留档,没有改写
  v2 的安全门。选择、配置与冻结验收证据提交为 `5cd0a418`。
- 冻结 GT 验收保持失败项可见:v2 完整合并 `17/20`、Recall@1 `488/577=0.8458`、
  高置信跨锚误合并 0;v3 为 `16/20`、`489/577=0.8475`、跨锚误合并 0。两者均通过
  完整合并门,均未达到 Recall@1 `0.86` 门;确认文件未枚举 hard-negative pairs,
  因此 `0/0` 不包装成四组硬负验收。baseline、v2、v3 的逐机会神谕并集也只有
  `495/577=0.8579`,继续调现有 blend 无法诚实越门。
- 当前代码对 4,629 条正式轨迹的独立复跑得到规范化 hash
  `5bf9364cf6e19a33ec59df784cc0543506547190ef6623eab08ee30e05592d1b`,与两次留档
  run 完全一致;专项测试 `42 passed`,全仓 `351 passed`,`compileall`、三个 CLI
  入口与 `git diff --check` 均通过。34 MiB 逐视角 NPZ 和大体积逐候选运行明细保留
  在本地成果目录,Git 只收录内容哈希、选择报告、指标与冻结判卷,避免把可再生产物
  当源码提交。
...
- 多视角重排把完整合并从 `14/20` 提高到 `17/20`,但没有跨过 Recall@1 门;代理 R@5
  安全门与冻结 GT 的 R@1 目标并不等价。三个现有工作点的神谕并集仍低于 0.86,说明
  继续微调 blend 只会消耗评测自由度;后续必须引入新的辨别证据或召回路径,并继续用
  冻结独立判卷限制回调。
