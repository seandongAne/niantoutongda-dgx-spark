...
> 赛程日：D4
>
> 我的当日主责：S3 跨视频匹配、四 Agent 审计链与 SF1-L1 轻量投影头
>
> 状态：进行中（四 Agent trace/单命令回放与 SF1-L1 dev_a 诊断闭环；A1 正式批次并行运行；正式素材待到）

## 今日目标

- 我先启动 S3 跨视频匹配关键路径，用 `dev-a-vocab5` 四视频现有产物形成可复跑基线，并逐组检查 A–D 四组同类不同实例硬负样本。
- 我会落地 S2.5 推理侧核心修复，冻结参数后只重跑一次 `dev-a-vocab6`，与 v5 对照。
- 我会把 `vlm_attributes` 主路切换为 Nemotron VL NVFP4-QAD/vLLM，保留 BF16 fallback，并裁撤独立 9B `task_copy` 槽位。
- 我把 A1 评价目标从额度消耗改为覆盖完整性与统计稳定性，建立无需人工逐条标注的
  合成旁白云/本地同题 A/B；赛方 SSH 限流公告后停止音频传输，改为 Spark 原地工厂。
- 我用现有 fixture 打通四 Agent 的完整可追溯消息链、严格校验与单命令回放，使正式素材到达后只替换输入，不重写审计协议。
- 我会在冻结 DINOv2 表征上训练 SF1-L1 轻量 metric projection head，以现有 `dev_a` 完成无泄漏切分、GB10 多种子训练、ReID 接线和失败门校准。
...
- 未验证或降级边界：
  ...
  - S2.5-6 云视觉教师/增强数据工厂在没有得到我对本批影像出境的明确同意前，不会上传家庭 crop；不阻塞 v6 主链。
  - A1 自动基准已落地确定性真值、3 类话术、1–3 物品、全字段/省略字段、双音色、
    5 类音频条件、统一 scorer 与 95% Wilson 停止门；正式 A/B 两端冻结零-shot、
    温度 0。实现 commit：`a885291`、`5056ddc`、`7ffa671`。
  - `calibration-20260716-v1` 暴露“先说去向再命名物品”的真值歧义，主动废弃且保留
    失败证据；修正后的 v2 云校准为 18/18 schema 合法、物品召回/精度 100%、字段
    准确率 173/180=96.111%，但每条件仅 6 条，距冻结门 147 条仍差 141 条，明确
    `stopping.reached=false`。用量为 6 次 TTS、18 次抽取、7616 prompt + 2807
    completion tokens，仅作预算账本而非成功 KPI。
  - 赛方公告 SSH 主动限流后，正在进行的合成 WAV rsync 已立即中止；新工厂在 Spark
    原地生成计划/TTS/扰动、先跑 StepFun 云端抽取，再移除内存 key、加载本地
    Step-Audio 2 mini，最终只拉回 64KB 判卷摘要。实现/安全/兼容/证据 commit：
    `891f502`、`c558104`、`9ae1e5e`、`9825b94`、`5332060`、`ac85435`、`3497a81`。
  - v3 Spark 原地校准已完整结束：云/本地均 18/18、覆盖率 100%、同题音频 hash
    18/18 一致，6 个 case 的 clean/noise20/speed090 hash 全部不同。云端 schema 100%、
    物品 P/R 100%/100%、字段 157/180=87.222%；本地 schema 17/18、P/R
    85.714%/100%、字段 161/180=89.444%；两端整条全对均 9/18。每条件仍仅 6/147，
    `stopping.reached=false`，2.22pp 差异不得解释为总体胜负。
  - v3 真实失败：云端系统性把“书桌上”缩成“书桌”，空间关系仍按槽位丢失记错；
    本地 mini 在 case-0001 的 noise20/speed090 把旧书桌、新书桌、学习用品过抽取为
  ...
  - 本地模型加载 124.865s、18 条均值 54.443s、峰值 CUDA 17,392,368,128 bytes；
    ...
    **138 passed**；codec32 已由 Spark 原地安装 aarch64 `imageio-ffmpeg` 并回环为
    16kHz PCM16。key 云阶段后进程副本已清除，平台 key 按我确认的安排在批次完成后 6 天删除。
  - 我已逐字确认更大暴露范围；`formal-20260716-v1-spark` 于
  ...
  - 四 Agent trace 已把 EXEC7、GROUP1、MEM5、SPACE4 和 USER 串成统一消息协议；fixture hero
    运行生成 20 条事件，覆盖 4 次 handoff、1 组澄清请求/选择结果、3 条验证链和 2 次人工确认。
  ...
  - 完整 ReID 用冻结四点网格 `match=0.80/0.82/0.84/0.86, margin=0` 校准，选择 0.80；
    ...
    `156→257`、澄清 `525→505`。但总门仍仅 `5/17 < 15/17`，所以我只把它记为
...
- A1 v1 证明“确定性生成”不等于真值天然无歧义；模板必须先做小批协议校准，发现歧义
  就换 run_id，不能在旧目录覆盖或用更高消耗掩盖 oracle 缺陷。
- 赛方 SSH 限流使“本地造音频再 rsync 到 Spark”失效；数据平面应贴着唯一 GPU 与国内
  API 网络原地运行，SSH 只留控制面和小摘要。与此同时，“API key disposable”不等于
  节点可信：曾受侵主机上的进程内存仍可能被同 UID/root 读取，必须新 key、显式接受
  暴露、调用上限、批后撤销四门齐全，不能因额度多而降低凭据纪律。
- A1 原地化首跑连续暴露两项环境假设：节点无 ffmpeg；新版 `torchaudio.load` 强依赖
...
- 审计链“有日志”不等于“可审计”：只有统一 message id、payload hash、causation、
  correlation、验证结果与人工确认都能闭合，回放器才能把缺路由、乱序和篡改判成失败。
...
- 正式素材到达后，我会将 fixture 输入替换为赛方素材并沿用同一 trace schema，跑完整 hero
  pipeline 后执行 `scripts/replay_trace.py ... --strict`；任何缺失 handoff、验证或人工确认都会阻断演示包。
- 我会按冻结的 `configs/sf1_hero_s1.yaml` 用正式 anchor 标注重训 SF1-L1，不沿用 dev_a 权重；
  用 `scripts/sf1_wire_reid.py` 生成带 SHA 锁的 ReID 配置，再复跑同一四点阈值网格与正式 G2 判卷。
- P0：我会完成 30–50 帧 task-A hardval 人工框与 17-anchor→tracklet 映射，补齐 v5/v6 三指标和正式四组硬负判卷；在此之前不调整验收结论。
- P1：我会在 S3 前增加低质量/重复候选门控，并先做同视频短轨缝合；目标是降低 921 条澄清，而不是简单放宽自动合并阈值。
- P1：我会把已产出的 hero crop 接入 NVFP4 属性抽取，形成颜色/材质/文字标记等多证据原型，再复验蓝色与玫红水壶等同类不同实例。
- P1：我会修正 S3 缺失属性的默认相似度语义，使 missing/unknown 不再产生恒定 1.0；属性产生方差且有标注验证后，才从 0 调高 `attribute/context` 权重。
- P2：hardval 证明 tile 有真实收益后，我才会保留按类/按场景触发；否则会缩减或关闭低运动 fallback，避免为候选膨胀支付 32.8% 墙钟成本。
- A1：我会持续监控 `formal-20260716-v1-spark`；云阶段完成后先验证 Spark 进程环境中的
...
- **我确认的两份真值均已落地**：17 锚点确认(161 轨,零重复归属、零硬负冲突,
...
- **GT 授权的阈值扫描**(12 组合 match×margin):全网格误合并=0、危险澄清恒=4;
  ...
  **v3 冻结待我目检 71 对新增链接**(`new-links-m084g000.jpg`;锚点 GT 只
...
## D4 深夜增量:我对产品方向的判断 — 家居视觉校准工厂

- **数据难度归因确认**:碎轨 0.206 / FP 17.4 是儿童房杂乱(遮挡/堆叠/同款多件)
  直接买单;完整合并 2/17 的嵌入天花板则与场景无关——两笔账分开记,
  dev_a 定位为压力测试集而非失败素材。
- **我认为产品方向应该升级**：我不希望每换一个房间都需要人工调整 GDINO；开发期以
  Step 3.7 Flash(教师:词表体系/易混组/失败归因)+ Step Image Edit 2
  (困难版本工厂)+ prompt_search + 真实留出集判断,持续产出版本化能力包
  household-pack-vN;运行时纯本地加载 pack,使用方式 = "自动盘点 + 轻确认"。
...
## D4 终增量:v3 冻结 + 正式验收(commit 0e20997)

- **我目检 71 对新增链接：71/71 对照正确** → v3 最后一道门解除。
  `configs/reid_dev_a_v3.yaml` 冻结(match 0.84 / margin 0.00,其余与 v2
  逐字段一致,frozen: true,校准依据全文写入配置注释)。
- **v3 正式验收**(spark,`s3-v6-v3-0e20997`):自动链接 156、澄清 525、
  3 次重复 hash 全同(501be077…),与扫描预测逐字段吻合;GT 正式判卷:
  硬负 4/4 PASS、误合并 0、R@1 0.8068(334/414)、完整合并 4/17
  (anchor_01/11/12/16)。两门未过属预期——结构解仍是属性接线。
- **我对命名错误机制的判断**（我发现“对照全对但命名很多错”）：量化结果
  = 161 条 GT 锚点轨检测类别 **0 错配**、71 对两端类别 **0 不一致**,
  但 142 端点中 110 个是无 GT 的非锚点轨,集中在箱体易混词
  (storage_box x24 / toy_storage x22 / mini_fridge x16——儿童房里
  mini_fridge 基本是玩具箱误认),另见跨 prompt 拼接畸形标签
  "baby monitor storage box" x2。结论:**误名不破坏链接(instance 0.80
...
## D4 P1 属性接线战报(commit 9fe3c30)

- **范围前提**：本战开打前，我已确定《初赛范围冻结 2026-07-16》——初赛唯一
  目标为单一受控英雄卧室任务,dev_a 降级为诊断证据;属性接线属"继续完成"
  清单(S3 低噪声聚合 + 实例级聚合证明),战役照打,判卷口径按诊断解读。
- **接线工程落地**(82 tests 绿):属性打分改白名单可比键
  (color/material/pattern/shape/text_marks),missing/unknown 键分子分母
  双剔除,零可比键 → None + 属性权重让渡(根除元数据混入打分的整类事故);
  enrichment 经 load_features 注入,stitch 合并组自动继承 hero 属性;
  reid_task/weight_sweep 全链带 --attributes 与 manifest sha。
- **S5 抽取全量跑通**(945/945 OK,0 错,9.7min,961 调用,15 轨升级补抽,
  局域 vLLM/NVFP4 8 并发,guided_json 服务端实测接受):主色 unknown 0%、
  材质 0.1%;text_marks 73% 空属预期。产物 results/s5/attributes_v6_s5v1.jsonl。
- **权重 A/B GT 判卷**(0.00/0.10/0.15/0.20,阈值锁 v3):全组误合并 0、
  硬负 4/4;R@1 0.807→**0.826**(@0.15,+2pp)、澄清 525→513;
  但完整合并 4→3。**机制**:碎轨的 hero crop 是局部视角(桌腿/床架袋),
  VLM 如实描述局部 → 同锚碎轨间属性天然不一致,属性分量反而惩罚真链;
  属性对"排序"是正贡献,对"碎链拼全"是负贡献。完整合并的结构瓶颈在
  碎轨与检测完整性,英雄场景合同(轮廓清楚/不遮挡/不堆叠)将天然消解。
- **命名权威大样本验证**:GDINO=mini_fridge 的 94 轨,VLM 独立命名后仅 8 轨
  仍是冰箱,其余散为 pet carrier/toy oven/storage bins 等——展示层污染
  规模实锤;锚点抽查 VLM 对完整物体命名准确(玩具冰箱→迷你冰箱、
  史迪奇玩偶→史迪奇毛绒玩具),对碎轨命名为局部(书桌→桌腿)——
  **实体级展示名应取全实体 hero_score 最高成员的 VLM 命名**,碎轨命名不上屏。
- **我的建议**：v3 仍为 dev_a 冻结口径；attr 0.15 作为英雄场景候选工作点
...
- **我认为 GROUP 不应依赖“画面空间邻近聚类”**——移动镜头二维远近≠真实
  空间关系；证据优先级定为 **旁白明确分组 > 人工轻确认 > 模板语义 >
...
- **GROUP 模块**(backend/tools/grouping/):旁白句式启发式切分 + 名称/颜色
  ...
  build_groups 使用四层取舍规则，轻确认与旁白冲突时旁白胜出并记录 conflict；
  未归组实体全部进轻确认队列。
...
## 增量 D4-5:等片窗口清仓——验收门/A1 选择/verify 切片/素材日就绪(深夜)
...
- **A1 本地化方案收官**：spark 建 stepaudio 独立 env,官方报文构造内联
  (三连败教训:相对导入/torchcodec/padding_mels 转置);四组对照
  (0.2/贪心 × 零-shot/few-shot)结论一致——**我选择的本地协议 = 零-shot 贪心**
  (判卷 1/3,label 3/3 全对,失分全在槽位保真);few-shot 两温度均 0/3
  且稳定幻化打包搭子“冬天的衣服”成独立物品，我不采用。定位结论:mini 边界=
  "听得准认得出",槽位结构化不达云参考 → 主链走"本地誊写+确定性解析器",
  mini 结构化只作对照。我的判断全文写入 `results/stepfun/a1_warmup/PROTOCOL.md`。
  我并行在建 TTS 语音鲁棒性测试农场（云 TTS 造噪声/口音变体）补判据。
...
- **verify 切片接进主链(G7b 就绪)**:AcceptanceManifest 合同(照片声明
  ...
  FAILED/NEEDS_USER 是合法业务结局，不改卡状态，留待人工确认，也不代写。
...
- 测试:**121 passed**(+9 acceptance,+更新主链 e2e 钉住 verify 阶段与
  三结局)。
- Commits:`b3c823e`(A1 选择)、`cc5815d`(verify 切片)、`6a99dbc`
  (s1+W1/W2 配置+阶段序修复)、`df70a2c`(词表生成器)。

## 增量 D4-6:我对 StepFun 工厂数据平面的分工选择(深夜)

- **背景**：赛方公告限流 SSH 文件传输（实测 17MB 音频半小时）。我认为合适的
  分工是：**音频工厂落节点（音频有数据引力，不可过境）；文本云调用走
  本地,key 不出 Mac**。
