...
- 未验证或降级边界：
  ...
  - 赛方公告 SSH 主动限流后，正在进行的合成 WAV rsync 已立即中止；新工厂在 Spark
    ...
    Step-Audio 2 mini，最终只拉回小体积判卷摘要。实现/确认门 commit：`891f502`、
    `c558104`；小体积代码已部署，远端 COMMIT=`c558104c...`。
  - 工厂启动前 Spark 为 121GiB 总内存、46GiB used、75GiB available、0 swap，且无
    A1/Step-Audio 并发进程；本地全套 **125 passed**。正式 v3 尚未启动：现有 StepFun
    key 注入曾被安全门在本地执行前拒绝，故没有把凭据发送到曾受侵的 Spark，也没有
    伪造云/本地 A/B 结果。公开文档未说明单 key 调用上限或自动过期，启动器现强制
    `--acknowledge-key-exposure`，只允许新建一次性 key 且注入即按泄露、批后删除。
...
- A1：取得对“一次性 key 注入即视为泄露并在批后立即删除”的明确确认后，启动
  `calibration-20260716-v3-spark`；先完成 6×3 云/本地贪心同题校准并只拉回摘要。
  若任一端 schema、真值、音频哈希或判卷一致性不过门，修协议而不扩样；只有两端协议
  可信才生成每条件 147 条的正式计划，并以覆盖完整 + CI 稳定停止，不以额度剩余加跑。
...
- **Sean 目检 71 对新增链接:71/71 对照正确** → v3 最后一道门解除。
...
- **命名错误机制定案**(Sean 报告"对照全对但命名很多错"):量化结果
...
- **范围前提**:本战开打前 Sean 落定《初赛范围冻结 2026-07-16》——初赛唯一
