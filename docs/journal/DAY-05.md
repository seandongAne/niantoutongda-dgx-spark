...
- D6 继续只使用现有视频和已固化失败样本，补独立目标视图、跨类别排他和全局实例
  选择；严格五目标门通过前保留显式视觉裁定，不把人工结果混入自动生产口径。
...
## 增量 D5-9：冻结相对空间口径，搬后验收与现场安全转为非阻塞（深夜）

- 技术 closure 升至 `hero_s1_technical_closure_v2`。五区布局只要求视觉相对容量和
  区域关系；精确面积、净高、承重、门洞净宽与通道净宽从当前验收输入中撤出。
  书桌高度按范围确认记录为 120 cm `ASSUMED_PRIOR`，明确标注非实测、仅作视觉
  尺度语境，不支持承重、通行或安全结论。
- 搬后照片的职责收窄为可选的物理执行复核：只在需要证明人已经按任务卡摆放时，
  才补充照片与人工 presence/region 事实并运行 verification 消息族。当前执行器不
  直接解析图片像素，因此没有照片时既不伪造 `VERIFIED`，也不阻塞可信库存、空间、
  布局和任务卡技术闭环；预注册 G7b 已以带日期修订从本轮主门撤出。
- 儿童触及、通道绊倒和湿区关系的范围状态冻结为 `DEFERRED / blocking=false`。
  三条规则评估器和诊断产物保留，空事实仍如实输出 3 条 `NEEDS_USER`；成果页改用
  “已延期 / 非阻塞 / 诊断缺证据”展示，避免把现场安全事实误写成当前技术待办，
  也不把延期解释为“安全”。风险 scope 仍支持未来切回 `ACTIVE`。
- 技术闭环全链复跑结果：`hero_s1_technical_closure_v2`、20 件可信库存、5 个
  placement 单元、`PLAN_READY`、5 张任务卡覆盖 20/20；成果页新增机器可读的
  “比赛技术闭环口径”区块。trace 四消息主链回放 `PASS`，bundle 35 个产物逐项
