...
## D4 最终范围裁决：初赛英雄场景收敛（commit bfd7a80）

- **触发事实**：Sean 重新核对参赛指导，确认初赛交付物是项目成果、演示视频与
  十日谈，不要求现场盲测或产品级泛化证明。此前“家居视觉校准工厂”和正式任务 B
  路线由此降级为赛后备忘，不再是初赛关键路径；历史记录保留，不改写成从未发生。
- **明确停止六项**：5000 张网络/生成图片、GDINO 全参数微调、多房间通用性验证、
  R1 大规模主动学习、正式任务 B 盲测，以及“任意家庭、任意物品都能识别”的产品级证明。
  停止项不再获得新的素材、标注、云额度、Spark GPU 或工程时间。
- **新英雄场景合同**：只交付一个授权、整洁、受控的卧室任务；使用 12～15 件轮廓清楚、
  不遮挡、不堆叠的锚点，形成睡前、学习、收纳或充电三个边界清楚的生活组合。继续使用
  冻结 GDINO 推理配置，完成 S3 轻量聚合、组合、区域图、CP-SAT、任务卡与双证据验收 trace。
- **可信度边界**：允许剪辑演示节奏，但输入、配置、日志、metrics、截图与系统结果必须来自
  真实管线；至少完成一次稳定复跑，守住高置信误合并为 0。对外只主张“受控场景的任务级
  视觉记忆”，不宣称跨房间或任意物品泛化。
- **裁决与交接物**：`docs/初赛范围冻结_2026-07-16.md` 与
  `docs/NEXT_SESSION_PROMPT_初赛英雄场景收敛.md` 已落档，并同步覆盖产品设计、七日切片、
  S2.5、StepFun playbook 和视觉校准工厂中的冲突表述。
- **失败与教训**：先前先按产品级通用性设计，再回头核对初赛提交要求，导致 5000 图、
  主动学习和盲测方案一度进入讨论。以后新增高成本验证门前，先把赛制要求、评审介质和
  对外主张逐项对齐；受控场景只要真实、可追溯、诚实限定边界，就不是伪造。
- **后续唯一排序**：英雄链路 P0 → 受控素材 → 可点击成果与真实证据 → 3～4 分钟演示视频
  （核心闭环尽量压进 90 秒）→ 十日谈；不得把已停止六项重新写回计划。
