...
- **vLLM+NVFP4 迁移当夜落地并实测收官(拍板后 ~3 小时)**:容器 `nvcr.io/nvidia/vllm:26.06-py3` + NVFP4-QAD 权重(9.9G,ModelScope)。两枪启动(第一枪缺 `--trust-remote-code`),同工况对照冒烟(`scripts/vllm_smoke.py`,与 BF16 bench 同 prompt 同 crop):**文本 7.8→26.5、图文 1.7→25.4 tok/s(~15×,且 vLLM 列含 prefill 口径更保守)**,128-token 属性抽取从 ~75s 降到 ~5s,输出质量无回退。transformers+BF16 降为已验证兜底。
