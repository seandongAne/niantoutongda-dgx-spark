...
- 参赛技术稿重写为 `docs/参赛项目技术介绍.md`,41 行、约 2000 字,用一篇连续文章和
  一张紧凑技术栈表说明选题、四 Agent 实现路径、Spark 本地模型服务与可回放证据链;
  初稿提交 `56694ee0`,强化叙事提交 `2bc0dc77`。
...
- 队内复核后删除参赛稿中的 R@1 起止值,标题与主线收敛为“把一个家的秩序编译进另一
  个家”及 `3306→20→5`。调优亮点改用产物可直接核对的 446 个困难/不确定样本对、
  117 个伪身份、511 个训练样本、18 组工作点与澄清减少 51%,技术力度由系统规模、
  自动化深度和端到端闭环共同呈现。
