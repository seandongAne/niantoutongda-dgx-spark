...
- 当前结果来自一个授权、受控的“旧卧室 → 新卧室”英雄任务，不宣称跨家庭、跨月份或全屋泛化。
- 自动布局输出区域级建议，不是厘米级室内设计或施工图。
- `NOT_SEEN`、`SUSPECTED_DUPLICATE` 和 `NEW_SPACE_INCOMPATIBLE` 是正式状态；系统不会用生成文案掩盖证据不足。
- 搬后真实照片验收是可选扩展，正式配置默认关闭；现有技术闭环不等同于已经完成物理搬家验收。
- 原始家庭视频、凭据、模型权重、ONNX/TensorRT 大文件和本地 `.env` 均不入库。
- 演示主链本地运行；云模型只用于开发期候选与错误归因，不能直接改写真值或产品事实。
