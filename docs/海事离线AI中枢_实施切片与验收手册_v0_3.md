# 海事离线 AI 中枢：实施切片与验收手册 v0.3

> 配套方案：[《海事离线 AI 中枢：项目定义与验证计划 v0.3》](./海事离线AI中枢_项目定义与验证计划_v0_3.md)
>
> 队伍：念头通达
>
> 文档日期：2026-07-14（v0.3 架构收敛修订）
>
> 用途：把项目愿景转成十天赛程内可逐段交付、现场共同验收、失败时可降级的工程计划

**v0.3 变更摘要**（v0.2 摘要见上一版文档）：架构收敛为 **4+1 Agent**（四异构主链 + 一条件声学，废除"七 Agent"口径，定义见项目定义 §4.0）；S6 改为**解释 Agent + 确定性引用审计门**（审计徽章、`audit_status`、对抗注入 20/20 全部保留，移除独立审计 LLM，常驻内存减 6–9 GB）；SF1 扩为**三层模型调优**（L1 微调 / L2 TensorRT / L3 工作点消融）并新增**场景隔离**红线；**协同 trace** 升格为一等验收产物（`agent_trace.jsonl`，融合→视觉复核请求显式化，S4 起生效）；SP0 中 Step-Audio 2 mini 必测、TTS-3B 降为 30 分钟限时保险探针；§3.4 门槛表新增协同 trace 完整性与训练/验证隔离两行；V1 分镜改为 4+1 架构图 + trace 动画；修复与项目定义互链文件名。
...
LLM 不参与 AIS 解码、CPA/TCPA 数值计算、目标身份绑定或告警阈值判断。它只读取已经形成的结构化事件，负责解释依据、检索规则与生成核查清单；**确定性引用校验器**再对解释做溯源校验——v0.3 裁决：溯源校验是精确匹配问题，归确定性层，这是本原则的自然延伸，不是资源妥协。
...
**主干必须完成：** AIS 回放→轨迹→视频检测→目标关联→风险提示→人工确认→审计→**解释 Agent + 确定性引用审计门**（v0.3：属主干，见 1.5）。
...
2. **S6 解释 Agent + 确定性引用审计门**：系统中 LLM 的唯一出场点，承接"可核验解释"差异化；审计徽章由确定性校验器点亮；模板回退仅为运行时故障保险，不是计划选项。
3. **SF1 三层模型调优**：L1 领域微调（主路径）+ L2 TensorRT 推理优化 + L3 工作点消融；目标**三层至少一层正收益**；L1 无提升须如实报告负结果（诚实红线），由 L2/L3 保证调优深度证据不悬空。
4. **双生态模型出场**：NVIDIA（TAO 检测器、Nemotron 系列）与 Stepfun 各至少一个模型在最终演示中真实运行（Stepfun **主选 Step-Audio 2 mini**，Step-Audio-TTS-3B 为平台适配保险），型号与版本写入 `configs/models.yaml`。
...
| A1 | 音频能力演示 | Step-Audio 2 mini 对公开真实声号的分类演示 | 公开真实录音就绪 | 缩小类别集合；Stepfun 出场由 TTS 兜底 |
...
| 解释首个有用内容 | P95 ≤2 s（流式渲染） | 完整解释 ≤8 s |
...
6. **Stepfun 出场验证（v0.3 修订）**：从 ModelScope 拉取 Step-Audio 2 mini（**必测**），完成一次可复现音频分类推理；Step-Audio-TTS-3B 为**限时保险探针**（P2，≤30 分钟）：跑通一句告警播报即记录、超时即停——保险须验证过才算保险，但不为它挤占主线时间。
...
- **NVIDIA 与 Stepfun 双生态各至少一个模型在 Spark 上完成一次可复现推理**（Stepfun 以 Step-Audio 2 mini 为准；TTS 探针结果如实记录，不作为门槛）；
...
船长可询问“为什么提示我”“哪些数据不确定”“应检查什么”，系统依据结构化事件、COLREG 摘要和船方 SOP 给出可追溯解释；每条解释附带**审计徽章**——确定性引用校验门通过才上屏。

### 架构与设计裁决（v0.3）

- **解释 Agent（船长助手 Agent 的生成侧）**：默认 Nemotron-3-Nano-30B-A3B（NVFP4，MoE 稀疏激活、解码快，契合带宽约束）；备选 Nemotron Nano 9B v2（官方 DGX Spark NIM 容器）。按告警触发，输出 ≤150 token、流式渲染；追问再展开。
- **确定性引用审计门（零模型）**，校验器规格：
  - 解释必须按下方结构化 schema 生成，自由文本不进审计门；
  ...
  - **违规处理**：打回重生成一次→仍违规→输出 `INSUFFICIENT_EVIDENCE` 并回退确定性模板（`audit_status=FALLBACK_TEMPLATE`，如实显示）。
- 审计**不阻塞告警展示**：告警的确定性字段（目标、CPA/TCPA、数据年龄）即时上屏；解释流式补充；审计徽章同步点亮（校验为毫秒级精确匹配）。
- **为什么不用第二个 LLM 审计（答辩统一口径）**：数字与引用溯源是精确匹配问题，确定性校验器给出构造性 100% 保证；LLM 审计反而引入概率性漏检，且违反 §1.1"先确定性、后生成式"。这是差异化技术判断，不是资源妥协——顺带节省 6–9 GB 常驻内存与一个待验模型。共权重、异系统提示的第二 LLM 视角仅在 D8 前全部切片绿灯时作为可选加深，不进计划。
...
source_refs[]
prohibited_actions[]
audit_status          # PASSED / REGENERATED / FALLBACK_TEMPLATE
```
...
- 解释请求、审计结果与徽章状态作为 trace 终段消息写入 `agent_trace.jsonl`。
...
| 90 秒主闭环 | ~90 s | S7 固化的完整回放：离线状态→AIS 态势→视频检测→融合（协同时间线）→告警→解释流式 + 审计徽章→人工确认→审计留痕 |
| 断网高光 | ~15 s | iptables 出站封锁规则特写 + 系统持续运行 |
| 调优对比 | ~20 s | SF1 三层证据：前后错误案例对比图 + 指标条（L1）、延迟/FPS 条（L2）、工作点曲线一瞥（L3） |
| 能力演示 | ~15 s | A1 声号分类（未同步标注在画面上）；TTS 播报一条告警（**条件镜头：TTS 探针已跑通才纳入**，否则以界面告警特写替代） |
...
- 今天是 D3：今日补齐 D1（环境与骨架）、D2（选题收敛）后开始正常记录；v0.3 架构收敛决策本身即 D3 日志素材；
- D9 汇编成文：以"生死门→切片推进→调优→固化"为叙事线，如实呈现失败与降级——诚实的失败记录比顺风叙事更有征文价值；
...
| 引用审计判定 | `audit_status`、违规项列表 | 否（确定性校验器产出） |
...
| LLM 超时 | 使用模板解释，告警不延迟 |
| 解释未过审计门 | 打回重生成一次；仍违规→`INSUFFICIENT_EVIDENCE` + 模板回退，`audit_status` 如实标注 |
...
- [ ] 告警、人工确认和审计闭环；
- [ ] **解释 Agent + 确定性引用审计门端到端演示（含审计徽章与对抗注入 20/20 记录）**；
...
- [ ] 双生态模型清单：`models.yaml`（NVIDIA / Stepfun 型号、量化、版本；Stepfun 以 Step-Audio 2 mini 为准，TTS 探针结果在案）+ SP0 双生态启动证据；
