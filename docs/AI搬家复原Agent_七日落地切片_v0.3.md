...
> **2026-07-16 初赛范围覆盖：**正式任务 B 盲测、多房间泛化、5000 张网络/生成
> 图片、R1 主动学习与 Grounding DINO 全参数微调均已停止；当前只交付一个受控英雄
> 任务、成果、演示视频与十日谈。冲突处以
> [`初赛范围冻结_2026-07-16.md`](./初赛范围冻结_2026-07-16.md) 为准。
...
4. 用户介入必须受指标约束：受控英雄任务中二选一率不得超过 20%，主演示最多出现一次。
...
|---|---|---|---|---|
| Day 1（07-15/D3） | G0、SP0-core、S0 | 非技术队员完成受控英雄任务素材和必要真值；技术队员完成主链探针与仓库骨架；SP0-score 下载启动 | 一套可授权展示、可复跑的受控任务；主链模型（检测/嵌入）与 CP-SAT 能在 Spark 跑通；空流程 UI 能创建任务 | **G1：数据、SP0-core、schema 三者全部 PASS 才开 S1；SP0-score 允许顺延至 Day 2 日终** |
...
| Day 6（07-20/D8） | S7、SF1、S8 第一轮 | 任务卡、二维码、验收图；TensorRT 与性能测量 | 从旧家视频到完成验收的端到端闭环；协同 trace 可单命令回放 | **G4：受控英雄任务完整跑通，证据链 100%；成品视频分镜冻结** |
| Day 7（07-21/D9） | S8 复验、V1 | 稳定复跑、失败镜头、旁白录制剪辑；E1 十日谈汇编 | 一键演示、指标页、3～4 分钟成品视频、提交包 | **G5：至少一次稳定复跑，无 OOM，无新增 P1；提交包按评分矩阵自检通过** |
...
| 数据与真值主责 | 拍摄受控英雄任务、物品实体/组合/区域必要标注、隐私脱敏 | manifest、绝对计数、错误案例 | 为演示手工修改系统实体、组合或布局结果 |
...
取得一个可合法展示、可重复运行的受控英雄任务，并为跨视频身份、生活组合和布局建立必要真值。

### 任务

1. 选择一个整洁、光线稳定、物品不遮挡不堆叠的授权卧室作为受控英雄任务。
2. 拍摄至少三段旧家视频和一段新家视频。
3. 人工编号十二至十五件锚点物品；编号不可从画面直接可读。
4. 每件锚点物品至少出现在三段视频中的两段，至少十件出现在全部三段。
5. 可保留一至两组同类但外观可区分的物品，不主动制造极端困难负样本。
6. 物品完整入画、留有间隙；通过多视角而非遮挡堆叠证明跨视频聚合。
7. 预先确认睡前、学习、收纳或充电三个生活组合，不允许看到模型输出后再改真值迎合模型。
8. 为新家标注至少五个区域、区域属性、容量等级和硬约束。
9. 冻结至少五个布局单元：三个生活组合和至少两个独立锚点物品。
10. 记录授权、拍摄者、允许展示范围和删除时间。
11. 去除 EXIF、地址、定位、人脸、证件、门锁和其他私人信息。
12. 保存输入、配置、真值、日志和授权范围；不建立正式任务 B 或封存盲测答案。
...
- 受控英雄任务满足“3 段旧家视频 + 1 段新家视频 + 12～15 件物品”。
- 每个物品在各视频中的出现/未出现均有真值。
- 同一物品与相似不同物品均有明确关系标签。
- 生活组合和区域真值在模型运行前冻结。
- 需要报告模型指标时按完整视频划分训练视角与验证视角。
- 授权和隐私检查通过。
...
- 新家区域没有证据图；
- 数据含未授权私人信息；
- 演示结果经过手工改写，无法从真实管线复跑。

发生阻断时优先修复受控英雄链路，不扩张到多房间、正式盲测或大规模数据工程。
...
- 受控英雄任务的 12～15 件锚点中至少 12 件被看到并形成轨迹。
...
- 冻结骨干基线；
- 用受控英雄任务的正样本和少量同类不同实例样本训练投影头或末层；
- 比较 `Recall@1`、错误合并、漏合并和二选一率；
- 不训练 Grounding DINO 权重，不建立任务 B 泛化门。
...
- 受控英雄任务的训练/验证使用同一指标实现；任何前后对比都保持输入、指标和运行环境一致。
- 报告调优前后绝对值与相对值。
- 最终模型和阈值在录制成品视频前冻结；报告只限于该受控任务，不外推为跨房间泛化。
...
## 15. S8：受控英雄链路集成与验收（124～152h）

### 目标

在受控英雄任务上，从旧家三段视频开始完整运行到新家验收，不对系统产物进行开发者手工修正。
...
受控英雄任务进入成品视频录制后，任何影响结果的配置变化都必须：

1. 生成新版本；
2. 重新运行完整 S8；
3. 同时报告旧版和新版；
4. 不删除不利结果；
5. 只允许修复 P0 正确性缺陷；修复后的运行必须标记为第二次评估，不能覆盖首次结果。
