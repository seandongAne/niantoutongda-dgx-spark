...
- 记录选题从海事离线 AI 中枢切换为 AI 搬家复原 Agent（团队一致投票）。
- 按赛事官方六项评审标准审计搬家方案的两份 v0.2 文档，并形成 v0.3 修订。
...
- 平台事实澄清：赛方节点为 24/7 SSH 资格，持续到 2026-07-22 赛程结束；唯一已知约束是跨境网络波动。已补记入 `CLAUDE.md` / `AGENTS.md` 远程机器信息。
...
- **两日主链冲刺启动**：方向收敛为技术先行，48 小时内目标 = 主链端到端（视频→实体→组合→区域→CP-SAT→任务卡→trace），真模型、在 Spark 上；冻结任务 B 验收、SF1 预注册、三次复跑等日历型证据流程仍按 v0.3 七日计划走。
...
- NVIDIA 模型机会核实：`nvidia/` 命名空间在 ModelScope 全 404，真实宿主为 **`nv-community/` 镜像 org**；`NVIDIA-Nemotron-Nano-12B-v2-VL-BF16`（→ vlm_attributes 槽位）与 `NVIDIA-Nemotron-Nano-9B-v2`（→ 任务文案润色槽位）均 API 核验 200 并已入队下载（`logs/nvidia_download.log`）；NVFP4-QAD 变体核验存在，记为量化/L2 升级路径不下载。models.yaml 两槽位已从 TBD 落到实 ID。
...
- **阶跃 Step Plan 接入（控制台截图显示配额 2000M，远超预期）**：落地 `scripts/stepfun_api.py` 本地客户端(stdlib-only,models/chat+audio 子命令,stderr 报 usage)+ `docs/STEPFUN_API_PLAYBOOK.md`(四条 dev-time 用途按冲刺价值排序:A1 prompt 预热 > G0 预标注/词表工厂 > LLM-judge > 文案润色;五条红线:密钥仅本地、演示主链不接云、云输出仅候选、影像出境先脱敏、批量任务记 usage)。`.env` 进 .gitignore 且 deploy.sh 排除——密钥双重隔离于 git 与节点。待 key 到手先跑 `models` 拿权威清单。

- **G0 试机片过真实管线（当晚拍摄 35s 卧室片段）**：67 关键帧、检测 0.40s/帧、大件与停留物全部成轨（bed 25 帧/12.5s 跨度）；72% 单帧碎片经标签分析确认为杂物闪烁（box×29）而非追踪失败——**碎片率对乱房间是误导性指标，真验收口径应为"每件停留锚点是否成长轨"**。宽松参数（iou 0.2/miss 4）缝合断轨且碎片 -25%，已定为 ingest 默认（真实批次后随 S3 阈值冻结）。`scripts/g0_clip_check.py` 成为素材质检门。房间分配为：A=我的房间（仅开发，素材不进任何提交物），B=队友房间（冻结+成片，需记录展示授权），另加 R1 鲁棒性扫描（队友房间 2–3 个，零标注，只验流程）。
...
- **codex 提案取舍（教师-学生循环 + TAO 微调 GDINO）**：诊断照单全收（其中指出 `detect.py` 跨批 NMS 无类别去重会误删堆叠实体的真缺陷），处方只采用一半——**TAO 微调设三条件门**（推理修复后仍不达标 ∧ 主链闭环 ∧ ≤Day 5；评分矩阵属"明确可舍弃"，单房间微调打冻结验收 = 过拟合撞枪口），Mask/SAM、R1 伪标签同缓。值得采纳的六项（canonical_id 词表编译 / alias-group NMS / prompt 自动搜索 / 停留段瓦片化 / 30–50 帧困难验证集 / 视觉教师+数据工厂分阶段损失）打包为 **S2.5 批次，一次性重跑 ingest**，专档文档 `docs/AI搬家复原Agent_S2.5_检测推理修复批次.md`。教训：**外部方案先过评分矩阵与泛化风险，再谈技术美感——"必须先修推理路径"这个判断是对的，但取舍仍需结合项目实际**。
...
- 会话勘误:本轮对话误开在 Brief-CC 仓库目录下(文件操作全走绝对路径,无实际影响);仓库根 `.env` 已补建(空 `STEPFUN_API_KEY=`,gitignore+deploy 双排除),待 key 到手后直接填入即可。项目记忆同步迁至本项目自己的 memory 存储,Brief-CC 侧只留指路桩。
- **Nemotron 服务路线调研 + BF16 tps 实测（问题：NVIDIA 有无原生适配？NVFP4 值不值得迁？）**：①原生适配存在但形态是 **vLLM 容器路线**(NVIDIA 官方 dgx-spark-playbooks;vLLM 内置 mamba2 内核,不需要 mamba-ssm/causal_conv1d — 已修复的 transformers 路线是模型卡的另一条官方路线,两个包是 NemotronH 混合架构的硬要求,不是配置错误);②NVFP4-QAD checkpoint 在 ModelScope 有镜像(已核验 200),官方配方 `vllm serve --quantization modelopt_fp4`;③**BF16 实测击穿 20 tok/s 判据:文本 7.8 / 图文(真实 S5 工况)1.7 tok/s** → 实测表明迁移条件成立，唯一堵点为 docker 组权限（Developer 在 sudo 组，运行 `sudo usermod -aG docker Developer` 即可解锁）。运行时三坑(auto_map 指 HF 原站→HF_ENDPOINT 镜像、triton 运行时 JIT 要 CPATH→.pth 注入且 sitecustomize 被 Debian 同名遮蔽、processor 的 num_patches 键)全部固化进 bootstrap/文档。调研与实测全档:`docs/NEMOTRON_SPARK_SERVING.md`。教训：**判断依据要先测再迁——"应该快很多"和"实测 1.7 tok/s 不可用"是两种说服力；而且 tps 判据要按真实工况（图文）取数，纯文本数字会乐观 4.6 倍**。
- **vLLM+NVFP4 迁移当夜落地并实测收官（迁移方向明确后约 3 小时）**：容器 `nvcr.io/nvidia/vllm:26.06-py3` + NVFP4-QAD 权重(9.9G,ModelScope)。两枪启动(第一枪缺 `--trust-remote-code`),同工况对照冒烟(`scripts/vllm_smoke.py`,与 BF16 bench 同 prompt 同 crop):**文本 7.8→26.5、图文 1.7→25.4 tok/s(~15×,且 vLLM 列含 prefill 口径更保守)**,128-token 属性抽取从 ~75s 降到 ~5s,输出质量无回退。transformers+BF16 降为已验证兜底。
- **codex 提速提案取舍（五项采用四项半）+ 单 tile A/B 验证**：tile 账精确命中（默认 5 tile prompt 1322 tok，单 tile 298 tok，提案预估 299）；但提案中的 `use_thumbnail:false` kwarg 是错的（NanoNemotronVLProcessor 不收，TypeError 拒启；源码 tile=1 时 thumbnail 自动不加，只传 `{"max_num_tiles":1}` 即可）。单请求 tok/s 25.4→26.0 基本持平——**收益不在单请求，而在批量：每调用省 ~1024 tok prefill + KV 足迹缩 4.4×**。取舍结果：#1 服务端已生效、letterbox 归 S5；#2 32-token JSON schema 归 S5（冒烟保 128 为可比性）；#3 hero 评分进 S2.5-8、条件升级归 S5；#4 服务参数已生效、并发扫等 S5 客户端；#5 缓存+指标重构归 S5 设计；A/B 顺序半保留（测量顺序对，工程顺序不对——提速不许插队 S3）。全档 `docs/NEMOTRON_SPARK_SERVING.md`。教训：**外部提案的算术可以全对，API 细节仍要上手核（源码 30 秒查清 kwarg 签名，省一轮盲试）**。
- **S2.5 批次扩容**：+S2.5-7 GDINO 帧批处理(0.4s/帧是管线真瓶颈,与瓦片化同文件同批施工,验收=bit-等价+墙钟下降)、+S2.5-8 hero crop 综合评分(面积×清晰度×截断惩罚);TensorRT 明确**维持 SF1-L2 原排期不提前**,进设门表。
...
- Day 1（切片日历）已在今日开始：G0 数据拍摄与真值冻结、SP0 双生态探针、S0 骨架三线并行；团队按手册 §2 分工。
