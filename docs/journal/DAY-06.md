...
- 基于冻结主链和已归档指标整理参赛技术介绍,供队友直接拆分整合;重点说明选题的
  技术定义、四 Agent 架构、模型栈、实现链、Spark 优化与诚实边界,不代写视频脚本。
...
- 新增 `docs/参赛项目技术介绍.md`,将选题从“看图盘点”重新表述为可计算的“生活
  状态迁移”:跨视频实例身份→可信物品记忆→生活关系图→新家区域图→约束布局→
  任务卡与可回放证据。正文聚焦技术部分,项目故事、完整简介和视频继续由队友材料承接。
- 架构稿明确四 Agent 权责、生成式与确定性边界、EXEC→MEM/SPACE 双路验收、
  AutoTune 的云端教练/Spark 训练/冻结真值裁判三权分离,并用 Mermaid 给出从旧家
  视频和旁白到新家任务卡的完整数据流。
- 技术栈只采用已实现或已有归档证据的组件:Grounding DINO、DINOv2+投影头、
  Step-Audio 2 mini、Nemotron VL NVFP4/vLLM、OR-Tools CP-SAT、Pydantic/JSONL/
  SHA-256 与内容寻址 bundle;未把设计期前端形态或当前未冻结的多视角 ReID 实验写成
  已交付能力。
- 对外数字逐项回查正式产物:3306→20、最多 4 问、3+2 组合覆盖 20/20、
  2278→168→5、独立空间 5/5、5 张任务卡、4 Agent replay `PASS`、44 产物 bundle、
  Nemotron 1.7→25.4 tok/s 与 AutoTune 0.8031→0.8083/澄清 -51%。所有引用 JSON
  均通过 `jq empty`,Markdown 无尾随空白,暂未使用工作区未提交结果。
- 诚实边界保留在正文主结构而非脚注:原始 ReID 14/20 与 R@1 0.8083 未达预注册门、
...
- 队友整合 `docs/参赛项目技术介绍.md` 时优先复用开头技术摘要、总体架构和已验证结果
  表;若正式主链或素材在整合前再次冻结,逐项刷新证据索引中的数字与边界,不从并行实验
  目录直接抄取未验收结果。
