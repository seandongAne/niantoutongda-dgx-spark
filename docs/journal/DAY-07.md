...
> 单锚点置信度 v2 通过既有无 GT 运行门并较三锚点对照净增 2 条 cycle closure,
> 但缺少新独立 GT,保持实验状态、不替换主工作点;Nemotron 3 Nano Omni 30B-A3B
> NVFP4 候选完成远端下载与结构校验,尚未加载或服务。
...
- 利用三段视频内物品静止先验,以稳定单实例周边物构造无 GT 局部拓扑指纹;
  ...
  生产门,不因结果放宽标准。
- 在三锚点邻域 v1 NO-GO 后试验单锚点安全版:引入局部锚点尺度、跨帧 MAD、共视率
  与证据置信度,使单个共同锚点只能提供低权重线索;固定算法后用 `min_shared=3`
  对照检验 1–2 锚点覆盖是否有独立贡献,并以三档 blend 生产网格决定是否保留。
...
### 增量 D7-7:单锚点置信度 v2、生产网格与三锚点消融(晚间)

- 邻域 sidecar 升级为 schema v2:目标-锚点距离同时按画面对角线与同帧锚点间局部
  尺度归一化,以距离 MAD、局部尺度覆盖、跨帧稳定度、共视率和共同帧数形成证据
  置信度;一个共同锚点的基础置信度上限为 `1/3`,候选实际 blend 再乘置信度。
  v1 sidecar 保持兼容,无证据候选与既有锁定组件保护不变。实现提交 `a8ae2f0e`。
- 单锚点代理覆盖 `27,133/30,788` 个候选,其中恰好一个共同锚点 `4,456`,平均
  置信度 `0.663814`。代理 winner 仍为 7 邻居/blend 0.30:pseudo R@1
  `0.443850`,R@5 `0.860963`,selection AUC `0.660428`,holdout AUC
  `0.711429`;旧单锚点算法的 selection/holdout AUC 为 `0.664884/0.697143`。
  新算法的 holdout 更稳但 selection 略低,代理结果不构成单调全面提升。
- 生产网格以 baseline `994` 条链接、`926` 条 automatic、`68` 条 cycle、`591`
  个澄清为参照。`min_shared=1, blend=0.10` 得到 `996/928/68/589`,匹配实体
  `595→596`,旧 926 条 automatic 零删除;两次完整回放 hash 均为
  `e1084827d5a3072cbd8f77e18f88521ecf1c112d3d9c9a0b5a70f388f48809a6`。
  blend 0.20/0.30 虽分别得到 995/997 条链接,但澄清升至 602/607,按预注册门
  判 FAIL;最终只冻结 0.10。
- 同算法、同 blend 的 `min_shared=3` 对照为 `994/928/66/589`;允许 1–2 锚点后
  自动模式与澄清不变,净增加 2 条 cycle closure,覆盖多 `9,141` 个候选、实际重排
  多 `1,870` 个。其中一条闭环直接来自共享锚点数 1 和 2 的两条 support pair,
  另一条由扩大的校准候选集合间接改变 assignment;新增两条 automatic 自身仍有
  5–6 个共同锚点。因此结论是“低锚点证据对闭环/校准有贡献”,不是“一个邻居可
  直接确认同一实体”。
- 配置冻结后才复用既有 20-anchor GT 做事后安全回归,且读取结果后未再调参:
  完整合并保持 `17/20`,高置信跨锚误合并保持 0,R@1 从 `488/577=0.8458`
  到 `489/577=0.8475`,仍未过 0.86 门;该集合已参与过历史工作且未提供硬负组,
  只说明已知样本未退化,不作为泛化精度证明。机器可读总判决位于
  `results/acceptance/HERO_S1/neighborhood-context-v2-production-gate-v1/report.json`,
  verdict=`PASS_EXISTING_OPERATIONAL_GATES_KEEP_EXPERIMENTAL`,证据提交 `ca33aeeb`。
  全量后端测试 `370 passed`,JSON/JSONL/YAML、SHA-256 与 `git diff --check` 通过。
...
- 单锚点不能从“有净链接收益”外推成“足以直接确认实体”。本轮净增量落在 cycle
  closure,且一条来自全局校准/assignment 的间接变化;必须同时审计边集合 churn
  与 support pair,不能只看总链接数。单锚点证据上限和置信度加权属于必要保护,
  不是独立 GT 的替代品。
- 代理 winner 的 blend 0.30 在生产网格使澄清增加 16,再次证明代理排序指标与
  运行目标并不等价。事后复用旧 GT 的 `+1` 命中也不能升格为精度结论:集合非独立、
  仍低于 0.86 门且没有硬负组;新工作点只能保持实验状态。
...
- 静态邻域 v1 保持 NO-GO;单锚点置信度 v2 仅冻结为实验候选,不替换主工作点。
  下一步采集新的未见三视频样本并预先冻结实体 GT/硬负组,比较 baseline、三锚点
  对照与单锚点 v2;验收前不在当前三视频继续调 blend。仍不确定的候选再交给
  高分辨率目标+周边 crop 或短视频片段模型裁决,输入显式列出目标及邻居实体。
