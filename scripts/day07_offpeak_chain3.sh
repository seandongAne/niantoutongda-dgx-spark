...
#   B) slice_scatter 最后一层二分(对比头 matmul vs scatter)
#   C) encoder-only BF16 扩集回归(revised-0.98 口径,32 帧)
...
echo "=== $(date -u +%FT%TZ) offpeak chain3 start commit=$COMMIT vllm_id=$VLLM_ID ==="
if [ -n "$VLLM_ID" ]; then
...
echo "=== $(date -u +%FT%TZ) stage A: v10 text-outside-export (container, fp32, runs 50) ==="
docker run --rm --gpus all --ipc=host \
  ...
  python scripts/gdino_torch_tensorrt_hybrid_probe.py \
    ...
    --output results/acceptance/SF1/torch-tensorrt-hybrid-20260719-v10-text-outside-export/probe.json \
...
echo "=== $(date -u +%FT%TZ) stage B: slice_scatter bisect ==="
"$PY" scripts/gdino_slice_scatter_bisect.py \
  --onnx "$V10DIR/grounding_dino.onnx" \
  --inputs "$V10DIR/sample_inputs.npz" \
  --engine results/acceptance/SF1/trt-scatter-bisect-20260719-v1/scatter_bisect_fp32_notf32.engine \
  --output results/acceptance/SF1/trt-scatter-bisect-20260719-v1/result.json \
  --trtexec "$TRTEXEC" \
  --code-commit "$COMMIT"
echo "STAGEB_EXIT=$?"
free -h

echo "=== $(date -u +%FT%TZ) stage C: encoder-only BF16 expanded regression ==="
FRAMES_DIR="$(find "$HOME/proj/results/hero_s1" -type d -name keyframes 2>/dev/null | head -1)"
[ -z "$FRAMES_DIR" ] && FRAMES_DIR="$HOME/proj/local-data/hero_s1"
echo "frames_dir=$FRAMES_DIR"
"$PY" scripts/gdino_selective_bf16_regression.py \
  --model-dir "$MODEL_DIR" \
  --frames-root "$FRAMES_DIR" \
  --max-frames 32 \
  --prompt "luggage. mini fridge. water bottle. desk." \
  --output results/acceptance/SF1/selective-bf16-regression-20260719-v1/regression.json \
  --code-commit "$COMMIT"
echo "STAGEC_EXIT=$?"
free -h
